	...
	class ERE_Captcha
	{
		...
		public function form_recaptcha() {
			...
			if (is_array($enable_captcha) && count($enable_captcha)>0) {
				...
				<div class="ere-recaptcha-wrap clearfix">
                    <?php wp_nonce_field('ere_recaptcha','ere_recaptcha_nonce') ?>
		...
		public function verify_recaptcha_wp_login($user, $username = '', $password = '') {
            ...
            if (!wp_verify_nonce($nonce,'ere_recaptcha')) {
                return new WP_Error( 'captcha_error',__( '<strong>Error</strong>: Captcha Invalid Security', 'essential-real-estate' ) );
            }
		...
		public function verify_recaptcha_wp_lostpassword($errors) {
            $nonce = isset($_POST['ere_recaptcha_nonce']) ? ere_clean(wp_unslash($_POST['ere_recaptcha_nonce'])) : '';
            if (!wp_verify_nonce($nonce,'ere_recaptcha')) {
                $errors->add( 'captcha_error', __( '<strong>Error</strong>: Captcha Invalid Security', 'essential-real-estate' ) );
            }
